	...
	public int insertBook(Book book) {
		...
		// sql문 만들기
				String sql = new StringBuilder()
						.append("insert into member (bookID, name, author, publisher, price)")
		...
		try {
			...
			// SQL문 매개변수에 값 추가
			jdbc.pstmt.setInt(1, book.getBookID());
			jdbc.pstmt.setString(2, book.getBookname());
		...
		public int updateBook(Book book) {
			...
			String sql = "set bookID = ?, name = ?, author = ?, publisher = ?, price = ?";
			int result = 0;
			try {
				jdbc.pstmt = jdbc.conn.prepareStatement(sql);
				jdbc.pstmt.setInt(1, book.getBookID());
